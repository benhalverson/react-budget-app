...
test('should set new date on date change', () => {
  const now = moment();
  const wrapper = shallow(<SingleDatePicker onDateChange={() => {console.log(wrapper)} }
    onFocusChange={onFocusChange => {}}
  />);
  wrapper.find(<SingleDatePicker/>)[0].prop('onDateChange')(now)
...
xtest('should set calendar focus on change', () => {
  const focused = true;
  const wrapper = shallow(<ExpenseForm />);
  wrapper.find('SingleDatePicker')[0].prop('onFocusChange')({ focused });
